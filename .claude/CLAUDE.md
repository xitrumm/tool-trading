...
## Kiến trúc file (7 khối, theo comment trong code)

| Khối | Thành phần | Vai trò |
|---|---|---|
| 0. Ghi log cmd | `_Tee`, `_log_file_for_now`, `_setup_logging` (gọi ngay khi import) | Tee `sys.stdout`/`sys.stderr` ra file `logs/bottrading_YYYY-MM-DD.log` (vẫn in cmd như cũ) — bắt cả `print` lẫn traceback lỗi, kèm timestamp giờ VN đầu mỗi dòng, tự tách file khi sang ngày mới |
...
- **Ghi log cmd ra file (Khối 0)**: `_setup_logging()` chạy NGAY lúc import (trước cả `load_config`) nên bắt được mọi `print` + traceback từ đầu. `_Tee` ghi đồng thời ra console gốc và file `logs/bottrading_<ngày>.log` (append, `utf-8` để giữ emoji/tiếng Việt), flush sau mỗi lần ghi để không mất log khi Ctrl+C/kill. **Tách file theo ngày kiểm tra ở MỖI lần ghi** (`_log_file_for_now`), không dùng cron — qua nửa đêm, dòng log đầu tiên của ngày mới tự mở file mới, KHÔNG cần restart (nếu im lặng qua nửa đêm thì file ngày mới chỉ tạo khi có dòng log đầu tiên). Lỗi ghi log bị nuốt (`except: pass`) để không làm sập bot. Thư mục `logs/` đã gitignore (có thể chứa nội dung tin/kèo nhạy cảm).
...
- **Log console khi broadcast** (`broadcast_to_bots`): mỗi tin có **`tag`** nhận diện (mặc định = dòng đầu của tin, cắt 40 ký tự — vd `[✅ KÈO THƯỜNG: SYN]`, `[⚠️ Market Status:]`) in kèm để KHÔNG bị quy nhầm cho coin vừa bị loại khi nhiều task async chen nhau (coin bị LOẠI `return` sớm, KHÔNG gọi hàm này nên KHÔNG có dòng gửi nào theo sau). Gửi **thành công thì im lặng** (không in theo từng người/bot) — chỉ in người LỖI (`🧹 ... gỡ <tên>` khi `blocked`/`deactivated`/`chat not found` → `remove_subscriber`, hoặc `⚠️ Bot ... → <tên> lỗi: ...`), rồi chốt 1 dòng `📤 [tag] Tổng đã phát: N tin`. Tham số **`announce=True`** in thêm dòng MỞ ĐẦU `📨 Bắt đầu phát tin: [tag]` (kèm timestamp giờ phát thực `datetime.now(VN_TZ)`) — **CHỈ** `auto_send_report` (BẢN TIN ĐIỂM TÂM VIP 07h/16h + gửi bù) truyền cờ này để tách hẳn khối báo cáo khỏi coin phía trên; các broadcast khác (kèo chốt, convergence, Market Status) để mặc định `announce=False`.
